...
function generateWeibullData(type: "cdf" | "pdf" | "hazard", shape: number, scale: number) {
  const data = []
  const maxTime = scale * 2 // Go up to twice the scale parameter
  ...
  for (let i = 0; i <= 100; i++) {
    ...
    switch (type) {
      ...
      case "pdf":
        ...
        value = (shape / scale) * Math.pow(time / scale, shape - 1) * Math.exp(-Math.pow(time / scale, shape))
        break
      case "hazard":
...
function generateMultiModeData(
  ...
  for (let i = 0; i <= 100; i++) {
    ...
    for (const mode of failureModes) {
      ...
      switch (type) {
        case "cdf":
          value = 1 - Math.exp(-Math.pow(time / mode.scale, mode.shape))
          break
        case "pdf":
          value =
            (mode.shape / mode.scale) *
            Math.pow(time / mode.scale, mode.shape - 1) *
            Math.exp(-Math.pow(time / mode.scale, mode.shape))
          break
        case "hazard":
          value = (mode.shape / mode.scale) * Math.pow(time / mode.scale, mode.shape - 1)
          break
      }
