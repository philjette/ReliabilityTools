...
/** After clamping shape, recompute scale from censored data: η = (β·S/r)^(1/β) */
...
function calculateWeibullCensoredMLE(
  ...
  // Profile-likelihood over shape (beta). Closed-form scale: eta^beta = beta*S/r => eta = (beta*S/r)^(1/beta)
  // log L = r*log(beta) - r*beta*log(eta) + (beta-1)*sum_fail log(t) - sum_all (t/eta)^beta  (sum_all = r/beta)
